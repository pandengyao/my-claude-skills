...
metadata:
  openclaw:
    ...
    install:
      - id: python-deps
        kind: exec
        command: "cd /root/.openclaw/workspace/skills/daily-hot-news && ~/py312/bin/pip install requests aiohttp"
        label: "安装Python依赖"
---
...
~/py312/bin/pip install requests aiohttp
```
...
```bash
# 查看已保存的数据统计
~/py312/bin/python3 storage.py

# 清理 30 天前的旧数据
~/py312/bin/python3 storage.py --clear 30
```
